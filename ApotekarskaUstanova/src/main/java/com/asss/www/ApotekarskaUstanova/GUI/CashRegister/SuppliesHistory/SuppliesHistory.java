/*
 * Created by JFormDesigner on Sat Mar 22 13:40:21 CET 2025
 */

package com.asss.www.ApotekarskaUstanova.GUI.CashRegister.SuppliesHistory;
 ...
 * @author lniko
 */
public class SuppliesHistory extends JFrame {
    public SuppliesHistory() {
        initComponents();
        this.objectMapper = new ObjectMapper();
        this.objectMapper.registerModule(new JavaTimeModule()); // Registrujte JavaTimeModule

        String[] columnNames = {"ID Prodaje", "Datum Dolaska", "Vreme Dolaska", "Dobavljac"};
    ...
    public static void start() {
        SwingUtilities.invokeLater(() -> {
            SuppliesHistory frame = new SuppliesHistory();
    ...
    private void datePickerPropertyChange(PropertyChangeEvent e) {
        LocalDate selectedDate = datePicker.getDate(); // Preuzmi datum
        if (selectedDate != null) { // Proveri da li je datum null
            LoadShipmentItems(selectedDate.toString()); // Pretvori datum u String i učitaj prodaje
        } else {
    ...
    private void LoadShipmentItems(String selectedDate) {
        ...
        try {
            // Fetch all shipments for the given date
            URL url = new URL("http://localhost:8080/api/shipment/date/" + selectedDate);
            ...
            if (connection.getResponseCode() == HttpURLConnection.HTTP_OK) {
                try (InputStream inputStream = connection.getInputStream();
                    ...
                    List<ShipmentDto> items = objectMapper.readValue(response, new TypeReference<List<ShipmentDto>>() {});

                    for (ShipmentDto item : items) {
                        String supplierName = "Nepoznati dobavljac"; // Default value
                        if (item.getSupplier() != null) {
                            supplierName = item.getSupplier().getName();
                        }

                        String data0 = String.valueOf(item.getId());
                        String data1 = String.valueOf(item.getArrivalDate());
                        String data2 = String.valueOf(item.getArrivalTime());

                        model.addRow(new Object[]{data0, data1, data2, supplierName});
                    }
                }
            } else {
    ...
    private String fetchSupplierName(long supplierId) {
        try {
            ...
            if (connection.getResponseCode() == HttpURLConnection.HTTP_OK) {
                try (Scanner scanner = new Scanner(connection.getInputStream())) {
                    String response = scanner.useDelimiter("\\A").next();
                    SupplierDto supplier = objectMapper.readValue(response, SupplierDto.class);

                    return supplier.getName();
                }
            } else {
                return "Nepoznati dobavljac";
            }
        } catch (Exception e) {
            e.printStackTrace();
            return "Greška pri učitavanju dobavljaca";
        }
    }
    ...
    private static int selectedShipment;
    private final ObjectMapper objectMapper; // Dodajte ObjectMapper kao polje klase

    public static int getSelectedShipment() {
        return selectedShipment;
    }

    public static void setSelectedShipment(int selectedShipment) {
        SuppliesHistory.selectedShipment = selectedShipment;
    }
}
